...
## 機能（CLIProxyAPI-Enhance）

### 使用統計と永続化
- 内蔵 SQLite によるリクエスト使用量の永続化（provider / model / auth 次元の集計）
- `/v0/management/usage` などの使用量 API
- 管理パネルの統計ページ（時間帯・provider・model・auth などでフィルタ）

### プロバイダーカスタムラベル
- AI プロバイダーに任意の表示名（`label`）を設定可能

### キーワードフィルタ
- 応答内容のキーワード一致時に失敗扱いし、負荷分散で当該プロバイダーを一時停止
...
remote-management:
  ...
  disable-auto-update-panel: false # パネル自動更新を無効化するか
```

本リポジトリは [router-for-me/CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI) の拡張フォークです。
