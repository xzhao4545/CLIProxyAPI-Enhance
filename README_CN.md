...
## 新增功能（CLIProxyAPI-Enhance）

### 使用统计与持久化
- 内置基于 SQLite 的请求用量统计持久化，支持按 provider、model、auth 等维度聚合查询
- 提供 `/v0/management/usage` 等用量查询接口
- 管理面板统计页支持按时间范围、provider、model、auth 等筛选

### Provider 自定义标签
- 可为 AI 提供商设置自定义名称（`label` 字段）

### 关键词过滤
- 可配置关键词规则；命中后将请求标记为失败，并在负载均衡中暂停该提供商
...
本仓库是 [router-for-me/CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI) 的增强分支，保留以上能力。
