...
The matched text is the original response text or a bounded excerpt around the matched keyword for large chunks. Matches are marked with code `keyword_filtered`, `Retryable: true`, and an internal HTTP status of `429`. Because this synthetic error has no provider `Retry-After`, it follows the consecutive transient-failure threshold instead of immediately marking the credential quota as exhausted. If more candidate models or credentials are available, the conductor can continue to the next candidate before the downstream HTTP response is committed. Stream-time matches are recorded as failed usage with the `keyword_filtered` code and the same message body. If the stream is filtered before the upstream emits token usage, the request still produces a zero-token failed usage record so management statistics show the failure.
